...
class paginador
	...
	function paginar_resultados($sql="")
		{
		//CALCULO EL REGISTRO DESDE EL CUAL SE EMPIEZA A ZONGAR LOS DATOS
		$inicio = $this->nro_pagina * $this->reg_paginas;
		
		//PAGINO LOS RESULTADOS
		$query_limit = sprintf("%s LIMIT %d, %d", $sql, $inicio, $this->reg_paginas);
		$this->datos_paginado = mysql_query($query_limit, $this->conect) or die('ERR PAG-63: NO SE HA PODIDO MOSTRAR LOS DATOS DEL LISTADO');
		
		//OBTENGO Y SETEO LA CANTIDAD TOTAL DE REGISTROS
		$all_sql = mysql_query("select count(*) from (".$sql.") as total", $this->conect) or die('ERR PAG-69: NO SE HA PODIDO ZONGAR LOS DATOS DEL LISTADO');
