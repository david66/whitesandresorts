...
while($gal = mysql_fetch_array($galeria)){
	...
	if($numimg==0){
		...
		$img_grande='dibujar.php?file=admin/archivos/'.$gal['aga_imagen'].'&ancho=560&alto=400&cut=1';

	}

	$arrgaleria[]= 'admin/archivos/'.$gal['aga_imagen'];

	$jsgaleria.= 'case '.$numimg.':document.getElementById(\'imagen_principal\').src="images/cargando.gif";document.getElementById(\'imagen_principal\').src="'.'dibujar.php?file=admin/archivos/'.$gal['aga_imagen'].'&ancho=560&alto=400&cut=1";break;';
	...
	if(is_array($arrgaleria))
		...
		if(count($arrgaleria)>1)
			...
			foreach($arrgaleria as $gal)
				...
				$ficha[$fila] .='<img id="img-mini" src="dibujar.php?file='.$gal.'&ancho=65&alto=49&cut=1" onclick="galeria('.$numimg.')" title="Click para agrandar">';
...
function cambiarPestana(tipo)
	{
	document.getElementById('imagen_principal').src="images/cargando.gif";
...
<link href="SpryAssets/SpryTabbedPanels.css" rel="stylesheet" type="text/css" />
 ...
 <div id="migas"><a href="index.php">WSR</a> > <a href="zonas.php"><?php echo $row['zon_nombre'];?></a> > <a href="<?php echo ($row['apa_estado']==1)?'modulos.php?zon='.$row['zon_id']:'usados.php?zon='.$row['zon_id'];?>"><?php echo ($row['apa_estado']==1)?_('Obra nueva'):_('Segunda mano');?></a> &gt; <a href="<?php echo $_SERVER['REQUEST_URI'];?>"><?php echo $row['apa_nombre'];?></a></div>
    ...
    <div id="TabbedPanels1" class="TabbedPanels">
      ...
      <div class="TabbedPanelsContentGroup">
        ...
        <div class="TabbedPanelsContent">
			...
			<?php // echo _('Zona: ').$row['zon_nombre'];?>

			<?php //echo _('Direccion: ').$row['apa_direccion'];?> <?php //echo $row['apa_cp'];?> <?php //echo $row['apa_ciudad'];?>  <?php //echo $row['apa_provincia'];?>
